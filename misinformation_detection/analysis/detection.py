...
class MisinformationDetector:
    ...
    def __init__(self):
        ...
        self.suspicious_patterns = {
            "miracle_cure": [
                "miracle cure", "miracle treatment", "miraculous", "100% effective",
                "guaranteed", "secret cure", "doctors hate this", "big pharma doesn't want you to know"
            ],
            "conspiracy": [
                "conspiracy", "cover up", "they don't want you to know", "hidden truth",
                "suppressed", "what they're hiding"
            ],
            "exaggeration": [
                "revolutionary", "breakthrough", "game-changer", "life-changing",
                "never seen before", "shocking results"
            ],
            "quick_results": [
                "instant results", "overnight", "immediate", "rapid", "quick",
                "fast results", "lose weight fast"
            ],
            "unverified_science": [
                "studies show", "research proves", "scientifically proven",
                "clinically proven", "doctors recommend", "experts agree"
            ]
        }
        ...
        try:
            ...
            analysis = {
                ...
                "explanation": self._generate_explanation(
                    combined_score, 
                    rule_based_results["patterns"], 
                    health_topics
                )
            }
            
            logger.info(f"Text analysis completed, misinformation score: {combined_score:.2f}")
            return analysis
        except Exception as e:
    ...
    def analyze_image_results(self, image_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            manipulation_score = image_analysis.get("manipulation_indicators", {}).get("score", 0.0)
            ...
            logger.info(f"Image analysis completed, misinformation score: {combined_score:.2f}")
            return analysis
        except Exception as e:
    ...
    def combine_content_analyses(
        ...
        try:
            ...
            if image_analyses and len(image_analyses) > 0:
                ...
                image_scores = [img.get("combined_score", 0.0) for img in image_analyses]
            ...
            if text_analysis and image_analyses:
                ...
                all_patterns = text_analysis.get("suspicious_patterns", []) + image_patterns
            elif text_analysis:
            ...
            logger.info(f"Combined analysis completed, misinformation score: {combined_score:.2f}")
            return analysis
        except Exception as e:
    ...
    def _get_model_score(self, text: str) -> Optional[float]:
        ...
        try:
            ...
            if len(tokens) > max_length:
                logger.info(f"Truncating text from {len(tokens)} tokens to {max_length}")
    ...
    def _generate_image_explanation(
        ...
        if manipulation_score > 0.7:
            explanation += "The image shows strong indicators of manipulation or editing.\n"
        elif manipulation_score > 0.4:
            explanation += "The image shows some indicators of possible manipulation or editing.\n"
        else:
        ...
        if text_present:
            ...
            if patterns:
                explanation += "The following suspicious patterns were found in the text:\n"
                for pattern in patterns:
                    explanation += f"- {pattern}\n"
            else:
