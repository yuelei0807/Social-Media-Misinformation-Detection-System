...
from misinformation_detection.models import Tweet, TwitterMedia, TikTokVideo, MisinformationAnalysis
...
def display_overview(db):
    ...
    misinformation_count = db.query(func.count(MisinformationAnalysis.id)).\
        filter(MisinformationAnalysis.is_potential_misinformation == True).scalar()
    ...
    with col4:
        misinformation_percentage = (misinformation_count / analysis_count * 100) if analysis_count > 0 else 0
    ...
    # Get recent analyses
    recent_analyses = db.query(MisinformationAnalysis).\
    ...
    if recent_analyses:
        ...
        for analysis in recent_analyses:
            content_id = analysis.tweet_id if analysis.tweet_id else analysis.tiktok_video_id
        ...
        st.info("No analysis data available. Start collecting and analyzing content to see results here.")
    ...
    dates = [datetime.datetime.now() - datetime.timedelta(days=x) for x in range(14, -1, -1)]
    scores = [0.3, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.6, 0.55, 0.5, 0.45, 0.5, 0.55, 0.6]
...
def display_twitter_analysis(db):
    ...
    text_only_count = db.query(func.count(Tweet.id)).filter(Tweet.has_media == False).scalar()
    with_media_count = db.query(func.count(Tweet.id)).filter(Tweet.has_media == True).scalar()
    ...
    top_tweets = db.query(Tweet).\
        order_by((Tweet.like_count + Tweet.retweet_count + Tweet.reply_count + Tweet.quote_count).desc()).\
        limit(10).all()
    ...
    misinformation_tweets = db.query(Tweet).\
    ...
    if misinformation_tweets:
        ...
        for tweet in misinformation_tweets:
            analysis = db.query(MisinformationAnalysis).\
            ...
            misinformation_data.append({
                ...
                "Engagement": tweet.like_count + tweet.retweet_count + tweet.reply_count + tweet.quote_count
            })
...
def display_tiktok_analysis(db):
    ...
    collector_stats = db.query(
        TikTokVideo.collected_by,
        func.count(TikTokVideo.id).label("count")
    ).group_by(TikTokVideo.collected_by).all()
    ...
    misinformation_tiktoks = db.query(TikTokVideo).\
        join(MisinformationAnalysis, TikTokVideo.id == MisinformationAnalysis.tiktok_video_id).\
        filter(MisinformationAnalysis.is_potential_misinformation == True).\
        order_by(MisinformationAnalysis.combined_score.desc()).\
        limit(10).all()
    ...
    if misinformation_tiktoks:
        ...
        for video in misinformation_tiktoks:
            ...
            misinformation_data.append({
                ...
                "Description": video.description[:50] + "..." if video.description and len(video.description) > 50 else video.description,
                "Score": analysis.combined_score if analysis else 0,
    ...
    # Example time series data
    # In a real implementation, you would query this from your database
    collection_dates = [datetime.datetime.now() - datetime.timedelta(days=x) for x in range(14, -1, -1)]
    collection_counts = [0, 1, 2, 0, 1, 0, 0, 3, 1, 0, 2, 1, 0, 0, 1]
    
    collection_data = pd.DataFrame({
        "Date": collection_dates,
        "Videos Collected": collection_counts
    })
...
def display_content_details(db):
    ...
    if content_type == "Twitter":
        ...
        if tweets:
            ...
            tweet_options = [f"{tweet.author_username}: {tweet.content[:30]}..." for tweet in tweets]
            selected_index = st.selectbox("Select a Tweet", range(len(tweet_options)), format_func=lambda x: tweet_options[x])
            ...
            if selected_index is not None:
                ...
                st.write(f"**Has Media:** {'Yes' if selected_tweet.has_media else 'No'}")
                ...
                # Get media items
                media_items = db.query(TwitterMedia).filter(TwitterMedia.tweet_id == selected_tweet.id).all()
                ...
                if media_items:
                    ...
                    for media in media_items:
                        ...
                        if media.local_path and st.button(f"View Media {media.id}"):
                            ...
                            st.info(f"Media would be displayed here from: {media.local_path}")
                
                # Get misinformation analysis
                analysis = db.query(MisinformationAnalysis).filter(MisinformationAnalysis.tweet_id == selected_tweet.id).first()
                ...
                if analysis:
                    ...
                    st.write("**Is Potential Misinformation:** ", "Yes" if analysis.is_potential_misinformation else "No")
        ...
        if tiktok_videos:
            ...
            tiktok_options = [f"{video.author_username}: {video.description[:30]}..." if video.description else f"{video.author_username}: [No description]" for video in tiktok_videos]
            selected_index = st.selectbox("Select a TikTok Video", range(len(tiktok_options)), format_func=lambda x: tiktok_options[x])
            ...
            if selected_index is not None:
                ...
                if selected_video.local_path and st.button("View Video"):
                    ...
                    st.info(f"Video would be played here from: {selected_video.local_path}")
                
                # Get misinformation analysis
                analysis = db.query(MisinformationAnalysis).filter(MisinformationAnalysis.tiktok_video_id == selected_video.id).first()
                ...
                if analysis:
                    ...
                    st.info("No misinformation analysis available for this TikTok video.")
