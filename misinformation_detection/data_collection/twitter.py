...
class TwitterCollector:
    ...
    def search_tweets(
        self, query: str, count: int = 100, result_type: str = "mixed"
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            tweets = self.api.search_tweets(
                q=query, count=count, result_type=result_type, tweet_mode="extended"
            )
    ...
    def download_media(
        self, media_url: str, tweet_id: str, media_type: str
    ) -> Optional[str]:
        """
    ...
    def save_tweet_to_db(self, tweet, db: Session) -> Tweet:
        ...
        try:
            ...
            has_media = (
                hasattr(tweet, "extended_entities")
            ...
            if has_media:
                for media in tweet.extended_entities["media"]:
                    ...
                    local_path = self.download_media(
                        media_url, tweet.id_str, media_type
                    )
        ...
        try:
            ...
            if has_media:
                category_path = os.path.join(
                    TWITTER_WITH_IMAGES_DIR, f"{tweet.id_str}.json"
                )
            else:
                category_path = os.path.join(
                    TWITTER_TEXT_ONLY_DIR, f"{tweet.id_str}.json"
                )
